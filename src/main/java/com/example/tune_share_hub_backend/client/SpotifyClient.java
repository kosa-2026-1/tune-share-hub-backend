...
public class SpotifyClient {
    ...
    @Value("${spotify.track-url}")
    private String trackUrl;

    public MusicSearchResponseDto getTrack(String trackId) {
        String accessToken = getAccessToken();

        SpotifyTrackSearchResponseDto.TrackItem response = restClientBuilder.build()
                .get()
                .uri(trackUrl + "/" + trackId)
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + accessToken)
                .header(HttpHeaders.ACCEPT_LANGUAGE, "ko-KR,ko;q=0.9,en-US;q=0.8,en;q=0.7")
                .retrieve()
                .body(SpotifyTrackSearchResponseDto.TrackItem.class);

        return SpotifyTrackConverter.toMusicSearchResponseDto(response);
    }
}
